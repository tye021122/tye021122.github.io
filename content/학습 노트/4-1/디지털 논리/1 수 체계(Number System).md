...
컴퓨터가 음수 부호를 인식하고 표현하기 위해 사용하는 세 가지 주요 방식이다
...
## 7.1 2의 보수 덧셈 및 뺄셈
* **2의 보수 덧셈:** 부호 비트를 포함하여 양수인 것처럼 일반적인 덧셈을 수행한다. 최상위 비트 밖으로 발생하는 올림수(Carry-out)는 무시한다 (단, 오버플로우 검사 필요)
...
* **소수부 변환 계산법:** 소수부에 2를 계속 곱하여 소수 부분이 0이 될 때까지 반복하며, 결과로 나오는 정수 부분을 위에서 아래 순서대로 나열한다
    * *예시: $0.625$의 이진수 변환*
...
각 비트 자리마다 고유한 가중치(값)가 정해져 있는 코드이다
